...
import Header from '@/components/TabHeader';
import { TaskContext } from '@/context/TaskContext';
import { useAppColors, useAppThemeMode } from '@/context/ThemeContext'; // Импортируем хуки темы и цвета
import { StatusData } from '@/data/StatusData';
import { completeTask } from '@/utils/taskUtils';
import { getFormatedDay, getTaskByDays } from '@/utils/utils';
import { RelativePathString, router } from "expo-router";
import { use, useCallback, useMemo, useState } from 'react'; // React 19: Добавили метод 'use'
import { StyleSheet, View } from 'react-native';
import { AgendaList, CalendarProvider, LocaleConfig } from 'react-native-calendars';
import { SafeAreaView } from 'react-native-safe-area-context';
...
const TaskListScreen = () => {
  // React 19: Используем хук 'use' для контекста
  const { task, setTask } = use(TaskContext);
  const [status, setStatus] = useState(StatusData.Upcoming.id);
  
  // Получаем глобальные стили цветов и режим темы
  ...
  // Адаптируем тему встроенных компонентов календаря под текущую цветовую палитру
  const providerCalendarTheme = useMemo(() => ({
    todayButtonTextColor: '#007aff',
    todayButtonFontWeight: 'bold' as const,
    // Настраиваем внутренности календаря под светлую/темную схемы
    calendarBackground: colors.containerBg,
    textSectionTitleColor: colors.metaText,
    dayTextColor: colors.titleText,
    monthTextColor: colors.titleText,
  }), [colors]);
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.containerBg }]}>
      ...
      >
        ...
        <AgendaList
          ...
          ListEmptyComponent={listEmptyComponent}
          renderItem={renderAgendaItem}
          // Передаем стили текста для липких заголовков самого AgendaList
          theme={{
            agendaDayTextColor: colors.sectionTitle,
            agendaDayNumColor: colors.sectionTitle,
