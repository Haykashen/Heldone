...
const TaskCardScreen = () => {
  ...
  const dateText = currTask.date ? currTask.date.toLocaleDateString() : 'Пусто';
  const timeText = currTask.date ? currTask.date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) : 'Пусто';
          ...
          <BottomSheetScrollView style={[styles.innerContainer, { backgroundColor: colors.containerBg }]}>
            <KeyboardAvoidingView style={styles.container}>
              ...
              <TextInput
                style={[
                  styles.titleInput,
                  { backgroundColor: colors.cardBg, color: colors.titleText, borderColor: colors.cardBg }, !currTask.title && styles.titleInputEmpty
                ]}
                value={currTask.title}
                onChangeText={changeTitle}
                placeholder="Название задачи"
              ...
              {/* ПОЛЕ ВВОДА ПРИМЕЧАНИЯ (CARD_INPUT) */}
              <TextInput
                style={[styles.cardInput, { backgroundColor: colors.cardBg, color: colors.titleText }]}
                value={currTask.notes}
                onChangeText={changeNotes}
                placeholder="Добавьте детали или описание задачи..."
                placeholderTextColor={colors.metaText}
