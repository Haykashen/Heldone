...
import { getSafeDateForPicker } from '@/utils/dateUtils';
import { copyPickedDocument, openFile, physicalDeleteFiles, shareFileWithCustomName } from '@/utils/fileUtils';
...
const TaskCardScreen = () => {
    ...
    const unsubscribe = navigation.addListener('beforeRemove', (e) => {
      ...
      startTransition(async () => {
        try {
          ...
          notifyMessage("Ошибка при авто-очистке файлов");
        } finally {
  ...
  const pickDocument = async () => {
    try {
      ...
      const fileCopyUri = await copyPickedDocument(pickedFile)
      addedFilesRef.current.push(fileCopyUri);
